...
def get_report_forms(text):
    ...
    for match in pattern.finditer(text):
        ...
        if form_match:
            forms[report_name] = form_match.group(1)
...
def get_quickview(report_body):
    ...
    return report_body[open_pos + 1:close_pos]
...
def get_fields_from_quickview(quickview):
    ...
    layout = quickview[layout_open + 1:layout_close]
    ...
    fields_body = layout[fields_open + 1:fields_close]
    ...
    for line in fields_body.splitlines():
        ...
        # allow an optional trailing comma after each field entry
        if line.endswith(","):
            line = line[:-1].strip()
...
def extract_reports(text):
    ...
    for match in pattern.finditer(reports_section):
        ...
        if close_pos == -1:
            print("WARNING: Could not close report:", report_name)
            continue

        report_body = reports_section[open_pos + 1:close_pos]

        quickview = get_quickview(report_body)
        fields = get_fields_from_quickview(quickview)
...
def main():
    ...
    print("Extracted:", len(results), "reports")
    ...
    for item in results:
        if item["report"] in [
            ...
            print(item["report"], "=>", len(item["fields"]), "fields")
